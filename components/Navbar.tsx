  ...
  return (
    <>
      ...
      <div className="bg-[#EB6D3A] h-10 flex items-center justify-between px-4 md:px-6 text-white text-sm">
        ...
        <div className="flex items-center">
          {[
            ...
            { icon: FaInstagram, href: "https://instagram.com/foundationfightingpoverty" },
            { icon: FaYoutube, href: "https://www.youtube.com/@foundationfightingpoverty3323" },
            { icon: FaLinkedinIn, href: "https://www.linkedin.com/company/foundation-fighting-poverty/" },
          ].map((social, idx, arr) => {
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white shadow-sm">
        <div className="hidden md:flex items-center space-x-2 text-[#009688] font-medium">
        ...
        <div className="hidden md:flex items-start space-x-2 text-[#009688] font-medium">
          <FaMapMarkerAlt className="text-[#EB6D3A]" />
          <p className="leading-snug text-sm">
            Head Office: 250-C, PECHS Block-06 <br /> Karachi, Pakistan
          </p>
        </div>

        {/*  Mobile Toggle */}
        <button
          className="md:hidden text-[#009688] text-2xl"
      ...
      {/*  DESKTOP NAV */}
      <nav className="hidden md:flex bg-[#009688] px-6 py-2 text-white sticky top-0 z-50 font-medium">
        <ul className="flex gap-8 items-center">
          ...
          >
            <Link
              href="/"
              className={isActive("/") ? "text-orange-200" : ""}
              onClick={() => setOpenDropdown("home")}
            >
              Home
            </Link>
            ...
            >
              {[
                ...
                <li key={item.href} className="px-5 py-2 hover:bg-gray-100 hover:text-[#EB6D3A] transition">
          ...
          >
            ...
            <ul
              className={`absolute left-0 mt-2 bg-white text-[#009688] shadow-xl rounded-lg w-56 transition-all duration-200 ${
                openDropdown === "work" ? "opacity-100 visible" : "opacity-0 invisible"
              }`}
            >
              <li className="px-5 py-2 hover:bg-gray-100"><Link href="/work#projects">Programs (16 Projects)</Link></li>
              <li className="px-5 py-2 hover:bg-gray-100"><Link href="/work#covid">Covid</Link></li>
              <li className="px-5 py-2 hover:bg-gray-100"><Link href="/work#flood">Flood</Link></li>
            </ul>
          </li>
          ...
          >
            ...
            <ul className={`absolute mt-2 bg-white text-[#009688] shadow-xl rounded-lg w-56 transition-all duration-200 ${
          ...
          <li
            ...
            onMouseLeave={() => setOpenDropdown(null)}
          >
            <Link href="/emergency" className={isActive("/emergency") ? "text-orange-200" : ""}>
              Emergency
            </Link>
          ...
          >
            ...
            <ul className={`absolute mt-2 bg-white text-[#009688] shadow-xl rounded-lg w-60 transition-all duration-200 ${
        ...
        <div className="ml-auto">
          <Link
            href="/donate"
            className="flex items-center gap-2 px-6 py-2 rounded-full font-semibold uppercase tracking-wide
      ...
      {mobileMenu && (
        <div className="md:hidden bg-[#009688] text-white text-center py-4 space-y-4 font-medium">
          <Link href="/" onClick={() => setMobileMenu(false)}><p className="py-2">Home</p></Link>
          <Link href="/work" onClick={() => setMobileMenu(false)}><p className="py-2">Work</p></Link>
          <Link href="/story" onClick={() => setMobileMenu(false)}><p className="py-2">Story</p></Link>
          <Link href="/emergency" onClick={() => setMobileMenu(false)}><p className="py-2">Emergency</p></Link>
          <Link href="/team" onClick={() => setMobileMenu(false)}><p className="py-2">Team</p></Link>
          <Link href="/donate" onClick={() => setMobileMenu(false)}><p className="py-2">Donate</p></Link>
          <Link href="/partners" onClick={() => setMobileMenu(false)}><p className="py-2">Partners</p></Link>
          <Link href="/contact" onClick={() => setMobileMenu(false)}><p className="py-2">Contact</p></Link>
        </div>
      )}
    </>
  );
